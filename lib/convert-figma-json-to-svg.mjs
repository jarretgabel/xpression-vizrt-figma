...
export function convertFigmaJsonToSvg(source, options) {
  ...
  function textBaselineOffset(style, node, box) {
    ...
    const isIgniteDisplayFamily = /^ESPN Ignite Display (?:Web|Sans)$/i.test(preferredFamily);
    const isNumericToken = /^\d+$/.test(characters.trim());
    const hasLetters = /\p{L}/u.test(characters);
    const leadingTrim = String(style.leadingTrim || '').toUpperCase();
    const renderBounds = node && node.absoluteRenderBounds;
    const renderBoundsBaselineNudge = 3 + fontSize * 0.08;
    const compactLabelBaselineNudge = hasLetters && !/^BentonSansCondBold$/i.test(preferredFamily)
      ? fontSize * 0.03
      : 0;
    let calibration = 0;
    ...
    if (isSingleLine && isNumericToken && isCompactLabel && isIgniteDisplayFamily) {
      calibration -= 0.1;
    }

    if (leadingTrim === 'CAP_HEIGHT' && isSingleLine && isCompactLabel) {
      let descentRatio = Number.isFinite(measuredMetrics && measuredMetrics.descentRatio)
        ? measuredMetrics.descentRatio
        : 0.22;
      if (isBentonFamily) {
        if (/^BentonSans(?:Bold|Black)$/i.test(preferredFamily)) {
          descentRatio += 0.08;
        } else if (/^BentonSansCond/i.test(preferredFamily)) {
          descentRatio += 0.04;
        } else {
          descentRatio += 0.035;
        }
      ...
      if (renderBounds && box
        && Number.isFinite(renderBounds.y)
        && Number.isFinite(renderBounds.height)) {
        const renderBottomOffset = (renderBounds.y + renderBounds.height) - box.y;
        return renderBottomOffset - descent + renderBoundsBaselineNudge + compactLabelBaselineNudge;
      }
      ...
      if (isIgniteDisplayFamily) {
        capHeightRatio = Math.max(capHeightRatio, 0.77);
      } else if (/^BentonSans(?:Bold|Black)$/i.test(preferredFamily)) {
        capHeightRatio = Math.max(capHeightRatio, 0.72);
      } else if (isBentonFamily) {
        capHeightRatio = Math.max(capHeightRatio, 0.7);
      }
    ...
    if (leadingTrim === 'CAP_HEIGHT' && isSingleLine && !isCompactLabel) {
      ...
      if (renderBounds && box
        && Number.isFinite(renderBounds.y)
        && Number.isFinite(renderBounds.height)) {
        const renderTopOffset = renderBounds.y - box.y;
        const renderBottomOffset = (renderBounds.y + renderBounds.height) - box.y;
        const baselineFromTop = renderTopOffset + ascent;
        const baselineFromBottom = renderBottomOffset - descent;
        return (baselineFromTop + baselineFromBottom) / 2 + renderBoundsBaselineNudge;
      }
